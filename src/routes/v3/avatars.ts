...
import { addAvatar, getAvatar } from "@/repositories/v3/avatars";
...
avatarsRouter.get(
  ...
  zValidator(
    ...
    z.object({
      userId: z
        .string()
        .max(64, { error: "UserId is too long (max 64 characters)" }),
    }),
...
avatarsRouter.post(
  "/",
  requireAuth("none"),
  zValidator("form", avatarSchema, handleValidationError),
  async (c) => {
